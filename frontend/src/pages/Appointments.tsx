...
// Helper function to format datetime in user's local timezone
        ...
        <CardContent sx={{ p: 3 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ fontWeight: 600 }}>
                  {formatLocalDate(appointment.time_start)}
                </Typography>
              </Box>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ fontWeight: 600 }}>
                  {formatLocalTimeOnly(appointment.time_start)}
                </Typography>
              </Box>
            </Box>
          </Box>
          ...
          {appointment.confirmation_code && (
            ...
            >
              ...
              <Typography
                variant="body2"
                sx={{ fontWeight: 600, color: "#840132" }}
              >
...
export default function AppointmentsPage() {
  ...
  const [clearCancelledDialogOpen, setClearCancelledDialogOpen] =
    useState(false);
    ...
    onSuccess: async () => {
      try {
        ...
        setErrorSnackbar({
          open: true,
          message:
            "Appointments cleared, but there was an error refreshing. Please refresh the page.",
        });
      }
    },
    onError: (error: any) => {
      console.error(
        "Failed to clear cancelled appointments - full error:",
        error,
      );
      ...
      let errorMessage =
        "Failed to clear cancelled appointments. Please try again.";
      ...
      try {
        if (!error) {
          ...
          if (typeof responseData === "string") {
            ...
            const msg =
              responseData.msg || responseData.error || responseData.title;
            errorMessage = msg
              ? String(msg)
      ...
      if (!errorMessage || errorMessage === "[object Object]") {
        errorMessage =
          "Failed to clear cancelled appointments. Please check the console for details.";
      }
  ...
  if (isAdmin) {
    return (
      <Box sx={{ maxWidth: 1400, mx: "auto" }}>
                ...
                filteredAppointments?.map((appt: any) => {
                  ...
                  return (
                    <TableRow
                      key={appt.id}
                      hover
                      sx={{ "&:hover": { bgcolor: alpha("#840132", 0.02) } }}
                    >
                      <TableCell>
                        <Chip
                          label={`#${appt.id}`}
                      ...
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                          {formatLocalDate(appt.time_start)}
                        </Typography>
                        <Typography
                          variant="caption"
                          sx={{ color: "text.secondary" }}
                        >
                          {formatLocalTimeOnly(appt.time_start)}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Chip
        ...
        >
              ...
              <>
                <Alert severity="info" sx={{ mt: 1, mb: 3, borderRadius: 2 }}>
                  <Typography variant="body2">
                    Patient: <strong>{selectedAppointment.user_name}</strong> (
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setEditProviderId(Number(e.target.value))
                      }
                    >
          ...
          <DialogActions sx={{ p: 3, pt: 2 }}>
            ...
            <Button
              ...
              sx={{ bgcolor: "#840132", "&:hover": { bgcolor: "#6a0129" } }}
            >
            ...
            {isLoadingSlots ? (
              ...
              <List>
                {timeSlotsData.slots.map((slot: any) => (
                  <ListItem key={slot.slot_id} disablePadding sx={{ mb: 1 }}>
                    <ListItemButton
                      ...
                      sx={{
                        borderRadius: 2,
                        border: "1px solid",
                        borderColor: slot.available ? "divider" : "transparent",
                        bgcolor: slot.available ? "transparent" : "#f5f5f5",
                      }}
                    >
                      <ListItemText
                        primary={`${formatLocalTimeOnly(slot.start)} - ${formatLocalTimeOnly(slot.end)}`}
          ...
          <DialogContent>
            {selectedAppointment && (
              <Typography>
                ...
                {formatLocalTime(selectedAppointment.time_start)}?
              </Typography>
            )}
          </DialogContent>
          <DialogActions sx={{ p: 2 }}>
        ...
        <Dialog
          open={clearCancelledDialogOpen}
          onClose={() => setClearCancelledDialogOpen(false)}
          PaperProps={{ sx: { borderRadius: 3 } }}
        >
          <DialogTitle sx={{ pb: 1 }}>
            <Typography variant="h6" sx={{ fontWeight: 700, color: "#d32f2f" }}>
          ...
          <DialogContent>
            <Typography>
              Are you sure you want to permanently remove all cancelled
              appointments from the system? This action cannot be undone.
            </Typography>
          ...
          <DialogActions sx={{ p: 2 }}>
            <Button onClick={() => setClearCancelledDialogOpen(false)}>
              Cancel
            </Button>
            <Button
              variant="contained"
    ...
    <Box sx={{ maxWidth: 1200, mx: "auto" }}>
      ...
      {!appointments || appointments.length === 0 ? (
        ...
        >
          ...
          <Typography variant="body2" sx={{ color: "text.disabled" }}>
            Use our AI Assistant to book your first appointment
          </Typography>
        </Paper>
      ) : (
        <Grid container spacing={3}>
          {appointments.map((appt: any, index: number) => (
            <Grid item xs={12} md={6} key={appt.id}>
              <AppointmentCard appointment={appt} index={index} />
            </Grid>
          ))}
        </Grid>
      )}
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to permanently remove all cancelled
            appointments from your view? This action cannot be undone.
          </Typography>
          {isAdmin && (
